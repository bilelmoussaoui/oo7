...
impl PrompterCallback {
    pub async fn new(window_id: &str, service: Service) -> Self {
    ...
    pub async fn set_secret_exchange_pub_key(&self, pub_key: String) {
        *self.secret_exchange_pub_key.write().await = pub_key;
    }

    pub async fn secret_exchange_pub_key(&self) -> String {
        self.secret_exchange_pub_key.read().await.clone()
    }

    pub async fn set_secret_exchange_aes_key(&self, aes_key: String) {
        *self.secret_exchange_aes_key.write().await = aes_key;
    }

    pub async fn secret_exchange_aes_key(&self) -> String {
        self.secret_exchange_aes_key.read().await.clone()
    }
    ...
    pub async fn prompt(
        ...
        match *reply {
            ...
            Some(Reply::Yes) => {
                ...
                if !locked {
                    if let Some(secret) = SecretExchange::retrieve_secret(
                        exchange,
                        &self.secret_exchange_aes_key().await,
                    ) {
                        match oo7::file::Keyring::open(&label, secret).await {
                            ...
                            Err(oo7::file::Error::IncorrectSecret) => {
                                ...
                                let server_exchange = self.secret_exchange_pub_key().await;
            ...
            None => {
                ...
                self.set_secret_exchange_pub_key(server_exchange.clone())
                    .await;
                ...
                if !locked {
                    ...
                    self.set_secret_exchange_aes_key(aes_key).await;
                }

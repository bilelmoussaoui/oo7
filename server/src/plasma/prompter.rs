...
#[cfg(test)]
static IS_PLASMA_DEFAULT: Option<bool> = Some(false);
#[cfg(not(test))]
static IS_PLASMA_DEFAULT: Option<bool> = None;

thread_local! {
    // This changes during test runs. By default it is false though, so we can find when we're expecting plasma without activating it first.
    pub(crate) static IS_PLASMA: LazyLock<Arc<Mutex<Option<bool>>>> = LazyLock::new(|| Arc::new(Mutex::new(IS_PLASMA_DEFAULT)));
}
#[must_use]
pub async fn in_plasma_environment(connection: &zbus::Connection) -> bool {
    let cached = IS_PLASMA.with(|cell| *cell.lock().unwrap());
    ...
    IS_PLASMA.with(|cell| cell.lock().unwrap().replace(is_plasma));
    is_plasma
}
...
impl PlasmaPrompterCallback {
    pub async fn accepted(&self, result_fd: OwnedFd) -> Result<CallbackAction, ServiceError> {
        ...
        let secret = {
            ...
            tracing::debug!(
                "Read secret from fd, length {}, buffer {}",
                buffer.len(),
                buffer
            );
